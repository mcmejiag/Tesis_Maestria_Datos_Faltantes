...
muestras_imputadas <- lapply(muestras_con_faltantes, function(Data) {
  ...
  Data$intubation <- as.numeric(Data$intubation)
  Data$Estado_general<-as.numeric(Data$Estado_general)
  Data$sexo<-as.numeric(Data$sexo)

  # Realizar imputación múltiple usando el método de regresión predictiva (bbpmm)
  imputation.data<-BBPMM(Data,nIter=4)
...
# Convertir variables numéricas a factores en un nuevo lapply
muestras_imputadas_factores <- lapply(muestras_imputadas, function(Data) {
  
  # Convertimos a factores (por si no lo están)
  Data$intubation <- factor(Data$intubation, levels = c(1, 2), labels = c("No", "Sí"))  # Usa el nombre exacto de tu columna
  Data$sexo <- factor(Data$sexo, levels = c(1, 2), labels = c("Femenino", "Masculino"))
  Data$Estado_general <- factor(Data$Estado_general, levels = c(1, 2, 3), 
                                labels = c("Bueno", "Regular", "Malo")) # Definimos orden
...
# Verifica la estructura de las muestras imputadas
str(muestras_imputadas_factores[[1]])

# Si deseas acceder a una muestra imputada específica, por ejemplo, la primera muestra:
summary(muestras_imputadas_factores[[1]])
View(muestras_imputadas_factores[[1]])

## 1 Femenino 2 Masculino
## 1 Si Intubacion 2 No intubacion
...
splits_list <- lapply(muestras_imputadas_factores, function(Data1) {
...
# Guardar como CSV
write.csv(resultados_auc, "resultados_auc_BBPMM.csv", row.names = FALSE)
...
# Realizar imputación múltiple para cada conjunto de datos en muestras_con_faltantes
muestras_imputadas_cv <- lapply(muestras_imputadas_factores, function(NewDataset) {
...
summary(muestras_imputadas_factores[[1]])
summary(muestras_imputadas_cv[[1]])
str(muestras_imputadas_cv[[1]])
...
for (i in 1:100) {
  ...
  train_data <- muestras_imputadas_cv[[i]] # Ajustar esto según el formato de tus datos imputados
...
write.csv(resultados_auc, "resultados_auc_BBPMM_CV.csv", row.names = FALSE)
