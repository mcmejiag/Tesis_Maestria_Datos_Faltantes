...
muestras_imputadas <- lapply(muestras_con_faltantes, function(Data) {
  ...
  # Realizar imputación múltiple usando el método de regresión predictiva (pmm)
  tempData <- mice(Data, m = 4, maxit = 50, meth = 'pmm', seed = 500)
...
# Guardar como CSV
write.csv(resultados_auc, "resultados_auc_PMM.csv", row.names = FALSE)
...
write.csv(resultados_auc, "resultados_auc_PMM_CV.csv", row.names = FALSE)
